...
- **A mutation survivor whose line was rewritten no longer sits in
  ...
  edit, push, revert); at HEAD, what is not committed is not there, an
  ...
  revert. And it asked the regeneration function, which swallows a
  SyntaxError and returns nothing, so a file that did not parse --
  conflict markers, a mid-edit save -- read as departed and every survivor
  in it was written `fixed` at once; departure is now decided by the line
  hashes alone, which need no parse, and a line that is present but cannot
  be regenerated is left alone. An absent, untracked, or escaping path is
  skipped (an absent file is `file_departed`'s case), and git failing to
  answer is never read as an empty file.
  A survivor recorded under an op the mutator cannot emit is never a
  candidate: the id is hashed with the op NAME, so a renamed or retired op
  can be regenerated by nobody, and clearing on it would write `fixed`
  for every survivor of the old name at the next push -- `mutation.OPS`
  now names every op in one place, and generation refuses a name outside
  it, so the list cannot lag the code silently. `aramid resolvers` grades
  the new resolver like the others.
...
[Unreleased]: https://github.com/jared0565/aramid/compare/v0.17.0...HEAD
...
[0.13.0]: https://github.com/jared0565/aramid/releases/tag/v0.13.0
