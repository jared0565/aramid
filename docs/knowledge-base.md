...
**Notices.** Two additional stderr notices fire per gate run, both pointing at `[tests].command` as the fix: (1) `detect_tests()` finds nothing despite a plausible test setup existing anyway (`tests/`, `test/`, `pytest.ini`, `tox.ini`, or `[tool.pytest.ini_options]` — e.g. a custom `python_files` pattern, unittest-style `testfoo.py` naming, or a doctest-only suite that the detector's literal filename match doesn't recognize); (2) both kinds are detected but no lockfile backs the npm side (the case above). A third, purely informational notice fires when a real dual-suite run's effective per-suite timeout already exceeds the shared gate budget, since the two suites share one `[timeouts]` deadline instead of each getting a full independent budget.
