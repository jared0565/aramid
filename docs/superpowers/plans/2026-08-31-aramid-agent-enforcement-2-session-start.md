...
Do NOT push as a plan step; hand back to the controller (full suite, then the integration menu). Sub-project 3 (pre-tool-use rejector + `arm --agent`) carries forward: adding its PreToolUse command to `TEMPLATE_COMMANDS` moves this sub-project's entry to "stale" on consumers — that is the designed rollout; the old single-command template joins `KNOWN_PRIOR_COMMANDS` at that moment. Also carried to sub-3: restoring the "Armed repos reject the call outright." sentence to the instruction-block template.
